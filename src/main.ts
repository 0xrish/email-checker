...
await Actor.main(async () => {
    ...
    if (cleanEmails.length === 0) {
        throw new Error('No valid emails found. Please provide a list of emails in the "emails" field.');
    }
    ...
    log.info(`Starting verification of ${cleanEmails.length} email(s) using backend "${backendUrl}"`);
